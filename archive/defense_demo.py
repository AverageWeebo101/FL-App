"""
Defense Demo — Enhanced Federated Learning Cycle
==================================================
...
Tab 1 — FL Simulation Replay
    ...
    Each click of "Next Round" animates client selection → validation →
    aggregation, then updates the live accuracy/AUC chart.

Tab 2 — Live Deepfake Detection
    Runs the trained TFLite model on a video file in real time,
    identical to deepfake_detector_video.py.
...
Usage:
    ...
    python defense_demo.py --model path/to/model.tflite
"""
...
ROUND_DATA = [
    ...
    (0,  0.9435, 0.9435, 0.9901, None,   None,   []),          # pre-train
    (1,  0.9478, 0.9478, 0.9924, 0.9504, 0.9496, ["006","004","005"]),
    (2,  0.9543, 0.9543, 0.9963, 0.9558, 0.9558, ["000","003","008"]),
    (3,  0.9543, 0.9543, 0.9963, 0.9558, 0.9558, ["002","007","009"]),  # R3 same trend
...
# Final reputation scores from ledger
CLIENT_REP = {
...
def resolve_model_path(given: str | None) -> str:
    """
    1. Use --model argument if provided and exists.
    2. Look for default name next to this script.
...
class DefenseDemo:
    ...
    def _build_ui(self):
        # Header bar
        header = tk.Frame(self.root, bg="#111111", pady=8)
        header.pack(fill=tk.X)
        tk.Label(
            header,
            text="An Enhanced Federated Cycle for DeepFake Detection  —  Defense Demo",
            bg="#111111", fg=HEX_TEXT, font=("Helvetica", 13, "bold")
        ).pack()
        ...
        style.configure("TNotebook.Tab",        background=HEX_PANEL, foreground=HEX_TEXT,
                         padding=[18, 6],       font=("Helvetica", 11, "bold"))
    ...
    def _build_fl_tab(self):
        ...
        # ── State ────────────────────────────────────────────
        self.fl_round_index = 0   # index into ROUND_DATA (0 = pre-train shown)
        self.fl_animating   = False

        # ── Layout: left panel | chart ────────────────────────
        left = tk.Frame(frame, bg=HEX_BG, width=370)
        left.pack(side=tk.LEFT, fill=tk.Y, padx=(10, 6), pady=8)
        left.pack_propagate(False)

        right = tk.Frame(frame, bg=HEX_BG)
        right.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0, 10), pady=8)

        # ── Round header ──────────────────────────────────────
        self.lbl_round = tk.Label(
            left, text="Pre-Training Baseline",
            bg=HEX_BG, fg=HEX_TEXT, font=("Helvetica", 14, "bold"),
            anchor="w"
        )
        self.lbl_round.pack(fill=tk.X, pady=(0, 4))

        self.lbl_phase = tk.Label(
            left, text="Global model initialised — no FL rounds applied yet.",
            bg=HEX_BG, fg="#888888", font=("Helvetica", 9),
            anchor="w", wraplength=350, justify="left"
        )
        self.lbl_phase.pack(fill=tk.X, pady=(0, 8))

        # ── Metrics row ───────────────────────────────────────
        metrics_frame = tk.Frame(left, bg=HEX_PANEL, relief=tk.FLAT)
        ...
        for col, (key, label) in enumerate([
            ...
            cell = tk.Frame(metrics_frame, bg=HEX_PANEL)
            cell.grid(row=0, column=col, padx=10, pady=8, sticky="nsew")
            metrics_frame.columnconfigure(col, weight=1)
            tk.Label(cell, text=label, bg=HEX_PANEL, fg="#888888",
                     font=("Helvetica", 8)).pack()
            var = tk.StringVar(value="—")
            self.metric_vars[key] = var
            tk.Label(cell, textvariable=var, bg=HEX_PANEL, fg=HEX_GREEN,
                     font=("Helvetica", 16, "bold")).pack()

        # Best checkpoint badge
        self.lbl_best = tk.Label(
            left, text="",
            bg=HEX_BG, fg=HEX_BEST, font=("Helvetica", 11, "bold"),
            anchor="w"
        )
        self.lbl_best.pack(fill=tk.X, pady=(0, 6))

        # ── Client grid ───────────────────────────────────────
        tk.Label(left, text="CLIENT POOL", bg=HEX_BG, fg="#666666",
                 font=("Helvetica", 8, "bold")).pack(anchor="w")

        client_grid = tk.Frame(left, bg=HEX_BG)
        client_grid.pack(fill=tk.X, pady=(2, 8))

        self.client_frames = {}
        self.client_rep_vars = {}
        for i, cid in enumerate(ALL_CLIENTS):
            row, col = divmod(i, 5)
            cell = tk.Frame(client_grid, bg=HEX_IDLE,
                            relief=tk.FLAT, padx=6, pady=6)
            cell.grid(row=row, column=col, padx=3, pady=3, sticky="nsew")
            client_grid.columnconfigure(col, weight=1)

            tk.Label(cell, text=f"C{cid}", bg=HEX_IDLE, fg=HEX_TEXT,
                     font=("Helvetica", 9, "bold")).pack()
            rep_var = tk.StringVar(value=f"R: {CLIENT_REP[cid]:.3f}")
            self.client_rep_vars[cid] = rep_var
            tk.Label(cell, textvariable=rep_var, bg=HEX_IDLE, fg="#888888",
                     font=("Helvetica", 7)).pack()
            self.client_frames[cid] = cell

        # ── Step indicator ────────────────────────────────────
        tk.Label(left, text="ROUND STEPS", bg=HEX_BG, fg="#666666",
                 font=("Helvetica", 8, "bold")).pack(anchor="w")
        ...
        step_frame = tk.Frame(left, bg=HEX_BG)
        step_frame.pack(fill=tk.X, pady=(2, 8))
        for key, label in steps:
            row = tk.Frame(step_frame, bg=HEX_BG)
            row.pack(fill=tk.X, pady=1)
            dot = tk.Label(row, text="●", bg=HEX_BG, fg="#333333",
                           font=("Helvetica", 10))
            dot.pack(side=tk.LEFT, padx=(0, 6))
            tk.Label(row, text=label, bg=HEX_BG, fg="#555555",
                     font=("Helvetica", 9)).pack(side=tk.LEFT)
            self.step_vars[key] = dot
        ...
        self.lbl_fl_status = tk.Label(
            ...
            bg=HEX_BG, fg="#666666", font=("Helvetica", 8),
    ...
    def _build_fl_chart(self, parent):
        ...
        self.fl_fig, self.fl_axes = plt.subplots(
            2, 1, figsize=(5.8, 6.2),
            facecolor=fig_bg, gridspec_kw={"hspace": 0.45}
        )
        ...
        ax_acc.set_ylim(0.92, 0.98)
        ax_acc.set_xticks(range(0, 11))
        ax_acc.set_xticklabels(
            ["Pre"] + [str(r) for r in range(1, 11)],
            color="#888888", fontsize=7
        )
        ax_acc.axhline(0.942, color="#555555", linewidth=0.8,
                       linestyle="--", label="FL-TENB4 baseline (94.2%)")
        ax_acc.legend(fontsize=7, facecolor="#333333",
        ...
        ax_auc.axhline(0.96, color="#555555", linewidth=0.8,
                       linestyle="--", label="FL-TENB4 baseline (0.96)")
        ax_auc.legend(fontsize=7, facecolor="#333333",
                      labelcolor="#888888", loc="lower right")

        # Live plot lines (start empty)
        ...
        # Best-round vertical markers (hidden until R6)
        self.vline_acc = ax_acc.axvline(x=-1, color=HEX_BEST,
                                         linewidth=1.5, linestyle=":",
                                         alpha=0.0)
        self.vline_auc = ax_auc.axvline(x=-1, color=HEX_BEST,
                                         linewidth=1.5, linestyle=":",
                                         alpha=0.0)
    ...
    def _fl_next_round(self):
        ...
        if next_idx >= len(ROUND_DATA):
            self.lbl_fl_status.config(
                text="All 10 rounds complete. Click Reset to replay."
            )
    ...
    def _fl_animate_round(self, idx):
        """Runs in a background thread; uses root.after to touch UI."""
        rd = ROUND_DATA[idx]
        selected = rd[6]

        steps_seq = [
            ("select",    f"Selecting clients: {', '.join('C'+c for c in selected)}", 0.9),
            ("train",     "Clients performing local training (5 epochs)…",             0.8),
            ("validate",  "Server validating updates — norm check + gain test…",       0.8),
            ("aggregate", "Aggregating with adaptive contribution weights…",            0.8),
            ("update",    "Updating reputation ledger for all 10 clients…",             0.6),
        ]

        def reset_steps():
            for key, dot in self.step_vars.items():
                dot.config(fg="#333333")

        def activate_step(step_key, msg, delay):
            self.root.after(0, lambda: self._fl_set_step(step_key, msg))
            time.sleep(delay)
    ...
    def _fl_set_step(self, active_key, msg):
        ...
        for key, dot in self.step_vars.items():
            dot.config(fg=colors[key] if key == active_key else "#333333")
        self.lbl_fl_status.config(text=msg)

    def _fl_highlight_clients(self, selected):
        for cid, cell in self.client_frames.items():
            if cid in selected:
                cell.config(bg=HEX_SELECTED)
                for child in cell.winfo_children():
                    child.config(bg=HEX_SELECTED)
            else:
                cell.config(bg=HEX_IDLE)
                for child in cell.winfo_children():
                    child.config(bg=HEX_IDLE)
    ...
    def _fl_update_display(self, idx):
        ...
        self.metric_vars["acc"].set(f"{acc*100:.2f}%")
        self.metric_vars["f1"].set(f"{f1*100:.2f}%")
        self.metric_vars["auc"].set(f"{auc:.4f}")

        # Best checkpoint label
        best_idx = max(range(len(ROUND_DATA)), key=lambda i: ROUND_DATA[i][1])
        best_rnd = ROUND_DATA[best_idx][0]
        if rnd >= best_rnd and best_rnd > 0:
            self.lbl_best.config(
                text=f"★  Best checkpoint: Round {best_rnd}  "
                     f"(Acc {ROUND_DATA[best_idx][1]*100:.2f}%)"
            )
        elif rnd == 0:
    ...
    def _fl_update_chart(self, idx):
        ...
        # Best-round vertical markers at Round 6
        best_rnd = 6
        if ROUND_DATA[idx][0] >= best_rnd:
            self.vline_acc.set_xdata([best_rnd, best_rnd])
            self.vline_auc.set_xdata([best_rnd, best_rnd])
            self.vline_acc.set_alpha(0.85)
            self.vline_auc.set_alpha(0.85)
    ...
    def _fl_finish_round(self, idx):
        ...
        if rnd == 6:
            ...
            self.lbl_fl_status.config(
                text="All rounds complete. Best-weight tracking deployed Round 6 model."
            )
            self.btn_next.config(state=tk.DISABLED)
            return
        else:
            self.lbl_fl_status.config(
                text=f"Round {rnd} complete. Click 'Next Round' to continue."
            )
    ...
    def _fl_reset(self):
        ...
        # Reset step dots
        for dot in self.step_vars.values():
            dot.config(fg="#333333")
    ...
    def _build_detection_tab(self):
        frame = tk.Frame(self.nb, bg=HEX_BG)
        self.nb.add(frame, text="  Live Detection  ")

        # ── Toolbar ──────────────────────────────────────────
        toolbar = tk.Frame(frame, bg=HEX_PANEL, pady=6)
        ...
        tk.Label(
            toolbar, text=f"Model: {model_name}",
            bg=HEX_PANEL, fg="#888888", font=("Helvetica", 9)
        ).pack(side=tk.RIGHT, padx=12)

        # ── Video canvas + stats ──────────────────────────────
        canvas_frame = tk.Frame(frame, bg=HEX_BG)
        canvas_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=(6, 0))

        self.det_canvas = tk.Label(canvas_frame, bg="black",
        ...
        self.det_lbl_verdict.pack(pady=(4, 2))
        ...
        tk.Label(stats_frame, text="FRAME TALLY", bg=HEX_PANEL,
                 fg="#888888", font=("Helvetica", 9, "bold")).pack()
        ...
        tk.Label(stats_frame, text="OVERALL VERDICT", bg=HEX_PANEL,
                 fg="#888888", font=("Helvetica", 9, "bold")).pack()
        ...
        # Keyboard shortcuts (only when detection tab active)
        self.root.bind("<space>",  lambda e: self._det_toggle_play()
                       if self.nb.index(self.nb.select()) == 1 else None)
...
def main():
    parser = argparse.ArgumentParser(
        description="Defense Demo — Enhanced FL Cycle"
    )
    parser.add_argument(
        "--model", default=None,
    ...
    root.geometry(f"+{max(0,(sw-ww)//2)}+{max(0,(sh-wh)//2)}")
