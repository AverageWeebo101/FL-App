"""
Defense Demo — Enhanced Federated Learning Cycle  (v4.1)
=========================================================
Changes from v4:
  - Fixed freeze-on-start: replaced threading.Event blocking chain in
    _fl_run_round with a pure callback chain on the main thread
  - Capped LedgerStrip height (148 px) so it never overflows the canvas area
  - Capped StepCard height (130 px) so it cannot push the chart off-screen

Changes from v3:
  - Larger network canvas (~900×560)
  - Step breadcrumb strip at top of canvas
  - Round counter badge on server node
  - Per-step infographic illustration cards (Option B: icon + headline + plain-English why)
  - Epoch counter ticking inside selected client nodes during training
  - Horizontal reputation ledger strip below canvas (prev/next round navigation)
  - Fixed auto-run stopping after Round 6 (fl_auto_paused now clears on resume)
  - FL-TENB4 baseline removed from chart

Tab order: Deepfake Detector (tab 1, dominant) | FL Simulation (tab 2)
"""
...
# Build per-round rep history for ledger (linear interpolation)
...
# Which rounds each client was selected
CLIENT_ROUNDS = {
...
# Per-step infographic card content (Option B)
STEP_CARDS = {
    "select": {
        "icon":     "🎯",
...
class NetworkCanvas:
    ...
    def __init__(self, parent, width, height):
        ...
        # Outer frame holds breadcrumb + canvas
        self._outer = tk.Frame(parent, bg=CV_BG)
        self._outer.pack(fill=tk.BOTH, expand=True)

        # ── Breadcrumb strip ─────────────────────────────────
        self._crumb_frame = tk.Frame(self._outer, bg="#0A0A18", pady=5)
        self._crumb_frame.pack(fill=tk.X)
        self._crumb_labels = {}
        for i, (key, label) in enumerate(STEPS_DEF):
            if i > 0:
                tk.Label(self._crumb_frame, text="›", bg="#0A0A18",
                         fg="#333355", font=("Helvetica", 11)).pack(side=tk.LEFT)
        ...
        # ── Step status bar ──────────────────────────────────
        self._status_bar = tk.Frame(self._outer, bg="#0A0A18", pady=4)
    ...
    # Dim palette for breadcrumb active-step background (solid dark tint)
    _CRUMB_BG = {
        "select":    "#302810",
        "train":     "#102030",
        "send":      "#301810",
    ...
    def set_breadcrumb(self, active_key):
        for key, lbl in self._crumb_labels.items():
            if key == active_key:
                ...
                lbl.config(fg="#333355", bg="#0A0A18")
    ...
    def animate_train(self, selected, on_done):
        ...
        total_steps = epochs * 8   # 8 animation ticks per epoch
        def grow(step=0):
    ...
    def animate_aggregate(self, selected, on_done):
        ...
        def pulse(i=0):
            if i < len(pulses):
                ...
                # Broadcast lines server→all clients
...
# ══════════════════════════════════════════════════════════════
# Reputation Ledger Strip
# ══════════════════════════════════════════════════════════════
class LedgerStrip:
    """
    Horizontal strip below the network canvas.
    Shows all 10 clients × columns: Client | Rounds Selected | Status | Rep Score
    Prev/Next arrows navigate which round's data is shown.
    """
    def __init__(self, parent):
        self._current_rnd = 0
        self._max_rnd     = 0

        outer = tk.Frame(parent, bg="#0A0A18", pady=6, height=148)
        outer.pack(fill=tk.X)
        outer.pack_propagate(False)  # cap height — header + 10 rows at font-7

        # Header row
        hdr = tk.Frame(outer, bg="#0A0A18")
        ...
        self._nav_lbl = tk.Label(hdr, text="Round 0 (Pre-training)",
                                 bg="#0A0A18", fg="#9090CC",
                                 font=("Helvetica",8))
        self._nav_lbl.pack(side=tk.LEFT, padx=(12,0))

        btn_cfg = dict(bg="#1A1A38", fg=C_TEXT, font=("Helvetica",9),
                       relief=tk.FLAT, padx=6, pady=1)
        self._btn_next = tk.Button(hdr, text="▶", command=self._go_next, **btn_cfg)
        self._btn_next.pack(side=tk.RIGHT, padx=(4,0))
        self._btn_prev = tk.Button(hdr, text="◀", command=self._go_prev, **btn_cfg)
        self._btn_prev.pack(side=tk.RIGHT, padx=(0,4))

        # Table frame
        tbl = tk.Frame(outer, bg="#0A0A18")
        tbl.pack(fill=tk.X, padx=10, pady=(4,0))

        # Column headers
        col_hdrs = ["Client","Rounds Selected","This Round","Rep Score","Δ Rep"]
        col_w    = [6, 18, 12, 10, 8]
        for j, (h, w) in enumerate(zip(col_hdrs, col_w)):
            tk.Label(tbl, text=h, bg="#0A0A18", fg="#666688",
                     font=("Helvetica",7,"bold"),
                     width=w, anchor="w").grid(row=0, column=j,
                                               padx=(0,6), pady=(0,2))

        # Data rows
        self._row_vars = {}  # cid → list of StringVar
        for i, cid in enumerate(ALL_CLIENTS):
            row_vars = [tk.StringVar() for _ in range(5)]
            self._row_vars[cid] = row_vars
            colors = [C_TEXT, "#AAAACC", C_TEXT, C_GREEN, C_TEXT]
            for j, (var, w, fg) in enumerate(zip(row_vars, col_w, colors)):
                lbl = tk.Label(tbl, textvariable=var,
                               bg="#0A0A18", fg=fg,
                               font=("Helvetica",7),
                               width=w, anchor="w")
                lbl.grid(row=i+1, column=j, padx=(0,6), pady=1)
                if j == 2:   # "This Round" column gets colour
                    self._row_vars[cid].append(lbl)  # store label ref at index 5
    ...
    def _refresh(self, rnd):
        ...
        self._nav_lbl.config(
            text=f"Round {rnd}" + (" (Pre-training)" if rnd == 0 else "")
                 + (" ★ Best" if rnd == 6 else ""))
...
class StepCard:
    """
    Infographic-style card (Option B).
    Shows: icon · headline · plain-English why · technical detail.
    Swaps content per step. Hidden when no step active.
    """
    def __init__(self, parent):
        self._frame = tk.Frame(parent, bg="#0D1A30",
                               relief=tk.FLAT, pady=8, padx=10, height=130)
        self._frame.pack(fill=tk.X, pady=(0,6))
        self._frame.pack_propagate(False)  # cap height so it never pushes chart off-screen

        top = tk.Frame(self._frame, bg="#0D1A30")
        top.pack(fill=tk.X)

        self._icon_lbl = tk.Label(top, text="", bg="#0D1A30", fg=C_TEXT,
                                   font=("Helvetica",18))
        self._icon_lbl.pack(side=tk.LEFT, padx=(0,8))

        right = tk.Frame(top, bg="#0D1A30")
        right.pack(side=tk.LEFT, fill=tk.X, expand=True)

        self._headline = tk.Label(right, text="", bg="#0D1A30", fg=C_TEXT,
                                   font=("Helvetica",10,"bold"), anchor="w")
        self._headline.pack(fill=tk.X)

        self._color_bar = tk.Frame(self._frame, bg="#333355", height=2)
        self._color_bar.pack(fill=tk.X, pady=(4,6))
        ...
        self._detail_lbl.pack(fill=tk.X, pady=(4,0))
    ...
    def clear(self):
        for lbl in (self._icon_lbl, self._headline,
                    self._why_lbl, self._detail_lbl):
            lbl.config(text="")
        self._color_bar.config(bg="#333355")
...
class DefenseDemo:
    def __init__(self, root, model_path):
        ...
        self.root.title("Enhanced FL Cycle — Defense Demo v4.1")
    ...
    def _build_detection_tab(self):
        ...
        tk.Label(badge, text=Path(self.model_path).name, bg="#0F3460",
        ...
        self.det_lbl_verdict = tk.Label(sp, text="—", bg=C_PANEL,
        ...
        self.det_lbl_overall = tk.Label(sp, text="—", bg=C_PANEL,
                                         fg=C_NEUTRAL, font=("Helvetica",22,"bold"))
    ...
    def _build_fl_tab(self):
        ...
        self.net_canvas = NetworkCanvas(canvas_host, width=900, height=520)
        ...
        tk.Button(ctrl, text="⟳  Reset", command=self._fl_reset,
                  bg="#2A2A4A", fg=C_TEXT, font=("Helvetica",9),
                  relief=tk.FLAT, padx=8, pady=6).pack(side=tk.LEFT)

        # Auto toggle
        af = tk.Frame(right, bg=C_BG)
        af.pack(fill=tk.X, pady=(0,4))
        self.auto_chk = tk.Checkbutton(
            af, text="Auto-run  (pauses at Round 6)",
            variable=self.fl_auto_mode,
            command=self._fl_on_auto_toggle,
            bg=C_BG, fg=C_TEXT, selectcolor="#2A2A4A",
            font=("Helvetica",9),
            activebackground=C_BG, activeforeground=C_TEXT)
        self.auto_chk.pack(side=tk.LEFT)
        self.lbl_auto_status = tk.Label(af, text="", bg=C_BG, fg=C_GOLD,
                                         font=("Helvetica",8,"bold"))
    ...
    def _build_fl_chart(self, parent):
        ...
        ax_acc.set_xlim(-0.5, 10.5); ax_acc.set_ylim(0.92, 0.975)
        ...
        ax_auc.set_xlim(-0.5, 10.5); ax_auc.set_ylim(0.985, 1.001)
        ax_auc.set_xticks(range(0,11))
        ax_auc.set_xticklabels(["Pre"]+[str(r) for r in range(1,11)],
                                color="#888888", fontsize=6)

        self.line_acc, = ax_acc.plot([], [], color="#4EA8F0", linewidth=1.8,
                                      marker="o", markersize=4, label="Accuracy", zorder=3)
        self.line_f1,  = ax_acc.plot([], [], color="#F07850", linewidth=1.2,
                                      linestyle="--", marker="s", markersize=3,
                                      label="F1", zorder=3)
        ax_acc.legend(fontsize=6, facecolor="#1E1E38", labelcolor="#888888",
    ...
    def _fl_next_round(self):
        ...
        # Resume after R6 pause — clear the paused flag
        ...
        # Call directly — _fl_run_round is now a pure callback chain
        # on the main thread; no background thread needed.
        self._fl_run_round(next_idx)

    def _fl_run_round(self, idx):
        """Pure callback chain on the main thread — no blocking Event."""
        rd       = ROUND_DATA[idx]
        selected = rd[6]
        rnd      = rd[0]
        ...
        # Each step shows its card then fires the animation;
        # the animation calls `next_fn` when done.
        def step_select():
            self.step_card.show("select")
            self.net_canvas.animate_select(selected, rnd, step_train)

        def step_train():
            self.step_card.show("train")
            self.net_canvas.animate_train(selected, step_send)

        def step_send():
            self.step_card.show("send")
            self.net_canvas.animate_send(selected, step_validate)

        def step_validate():
            self.step_card.show("validate")
            self.net_canvas.animate_validate(selected, step_aggregate)

        def step_aggregate():
            self.step_card.show("aggregate")
            self.net_canvas.animate_aggregate(selected, step_rep)
        ...
        # Kick off the chain — use after(0) so Tkinter finishes any
        # pending redraws before the first animation begins.
    ...
    def _fl_on_auto_toggle(self):
        if self.fl_auto_mode.get():
            ...
            # If we were paused at R6, toggling auto back on resumes
            if self.fl_auto_paused:
                self.fl_auto_paused = False
    ...
    def _det_update_stats(self,label,hc,cf,rp,sm,ms):
        self.det_lbl_verdict.config(text=label,fg=hc)
        self.det_lbl_confidence.config(text=f"Confidence: {cf:.1f}%",fg=hc)
        self.det_conf_bar["value"]=min(cf,100)
...
# ══════════════════════════════════════════════════════════════
def main():
    parser = argparse.ArgumentParser(description="Defense Demo v4")
