"""
Defense Demo — Enhanced Federated Learning Cycle  (v4)
=======================================================
Changes from v3:
        ...
        for key, lbl in self._crumb_labels.items():
            if key == active_key:
                lbl.config(fg=STEP_COLORS[key],
                           bg=STEP_COLORS[key]+"22" if len(STEP_COLORS[key])==7 else "#1A1A30")
            else:
...
class LedgerStrip:
    ...
    def __init__(self, parent):
        ...
        outer = tk.Frame(parent, bg="#0A0A18", pady=6)
...
class DefenseDemo:
    def __init__(self, root, model_path):
        ...
        self.root.title("Enhanced FL Cycle — Defense Demo v4")
    ...
    def _fl_run_round(self, idx):
        ...
        def run_step(key, anim_fn):
            done.clear()
            self.root.after(0, lambda k=key: self.step_card.show(k))
            self.root.after(0, lambda: anim_fn(on_done))
            done.wait()

        run_step("select",
                 lambda cb: self.net_canvas.animate_select(selected, rnd, cb))
        run_step("train",
        ...
        self.root.after(0, lambda: self.ledger.advance_to(rnd))
        self.root.after(0, lambda: self.step_card.clear())
        self.root.after(0, lambda: self.net_canvas.clear_breadcrumb())
