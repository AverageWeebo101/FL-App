"""
Defense Demo — Enhanced Federated Learning Cycle  (v5)
=======================================================
Changes from v4.1:
  - Mid-light "slate" theme: charcoal-slate UI, canvas stays dark
  - Ledger redesigned as two side-by-side columns of 5 clients → fits ~100 px, no clipping
  - Canvas enlarged to 940×560
  - StepCard moved beside the canvas (centre column), right panel is now metrics+chart only
  - Chart Y-axis tightened to actual data range ±0.002
  - Rep step shows numeric Δ pop-ups on every node (+0.003 / −0.002 etc.)
  - Round 6 metrics flash gold three times to draw the eye
  - Round 6 progress bar strip more prominent
...
class LedgerStrip:
    ...
    """
    _LEFT_CLIENTS  = ["000","001","002","003","004"]
    _RIGHT_CLIENTS = ["005","006","007","008","009"]

    def __init__(self, parent):
        self._current_rnd = 0
        self._max_rnd     = 0

        # Outer container — fixed height, no propagation
        outer = tk.Frame(parent, bg=C_PANEL, pady=4, height=110)
        ...
        col_hdrs = ["Client","Selected Rnds","Status","Rep","Δ Rep"]
        col_w    = [6, 14, 12, 8, 8]

        for side_idx, group in enumerate([self._LEFT_CLIENTS, self._RIGHT_CLIENTS]):
            tbl = tk.Frame(tables, bg=C_PANEL)
            tbl.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 10 if side_idx==0 else 0))

            # Column headers
            for j, (h, w) in enumerate(zip(col_hdrs, col_w)):
                tk.Label(tbl, text=h, bg=C_PANEL, fg="#888AAA",
                         font=("Helvetica",7,"bold"),
                         width=w, anchor="w").grid(row=0, column=j, padx=(0,4), pady=(0,1))
            ...
            for i, cid in enumerate(group):
                ...
                for j, (var, w, fg) in enumerate(zip(row_vars, col_w, colors)):
                    lbl = tk.Label(tbl, textvariable=var,
                                   ...
                                   font=("Helvetica",7),
                                   width=w, anchor="w")
                    lbl.grid(row=i+1, column=j, padx=(0,4), pady=0)
                    if j == 2:
...
class DefenseDemo:
    def __init__(self, root, model_path):
        ...
        self.root.title("Enhanced FL Cycle — Defense Demo v5")
    ...
    def _build_detection_tab(self):
        ...
        tk.Label(sp, text="CURRENT FRAME", bg=C_PANEL, fg=C_TEXT2,
    ...
    def _build_fl_tab(self):
        ...
        ctrl_area = tk.Frame(centre, bg=C_BG)
        ctrl_area.pack(fill=tk.X, pady=(6,0))

        ctrl = tk.Frame(ctrl_area, bg=C_BG)
